...
abstract class BaseCotisation extends BaseObject implements Persistent
{
    ...
    /**
     * The value for the personne_id field.
     * @var        int
     */
    protected $personne_id;

    /**
     * The value for the debut field.
     * @var        string
     */
    protected $debut;
    ...
    /**
     * Get the [personne_id] column value.
     *
     * @return int
     */
    ...
    /**
     * Get the [optionally formatted] temporal [debut] column value.
     *
     *
    ...
    public function getDebut($format = '%F')
    {
        ...
        if ($this->debut === '0000-00-00') {
            // while technically this is not a default value of null,
            // this seems to be closest in meaning.
            return null;
        } else {
            try {
                $dt = new DateTime($this->debut);
            } catch (Exception $x) {
                throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->debut, true), $x);
            }
        }
    ...
    /**
     * Set the value of [personne_id] column.
     *
     * @param int $v new value
     * @return Cotisation The current object (for fluent API support)
     */
    public function setPersonneId($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->personne_id !== $v) {
            $this->personne_id = $v;
            $this->modifiedColumns[] = CotisationPeer::PERSONNE_ID;
        }
    ...
    /**
     * Sets the value of [debut] column to a normalized version of the date/time value specified.
     *
    ...
    public function setDebut($v)
    {
        ...
        if ($this->debut !== null || $dt !== null) {
            $currentDateAsString = ($this->debut !== null && $tmpDt = new DateTime($this->debut)) ? $tmpDt->format('Y-m-d') : null;
            $newDateAsString = $dt ? $dt->format('Y-m-d') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->debut = $newDateAsString;
                $this->modifiedColumns[] = CotisationPeer::DEBUT;
            }
    ...
    public function setFin($v)
    {
        ...
        if ($this->fin !== null || $dt !== null) {
            $currentDateAsString = ($this->fin !== null && $tmpDt = new DateTime($this->fin)) ? $tmpDt->format('Y-m-d') : null;
            $newDateAsString = $dt ? $dt->format('Y-m-d') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->fin = $newDateAsString;
                $this->modifiedColumns[] = CotisationPeer::FIN;
            }
        } // if either are not null
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aPersonne !== null && $this->personne_id !== $this->aPersonne->getId()) {
            $this->aPersonne = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(CotisationPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`ID`';
        }
        if ($this->isColumnModified(CotisationPeer::PERSONNE_ID)) {
            $modifiedColumns[':p' . $index++]  = '`PERSONNE_ID`';
        }
        if ($this->isColumnModified(CotisationPeer::DEBUT)) {
            $modifiedColumns[':p' . $index++]  = '`DEBUT`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`ID`':
                        ...
                        break;
                    case '`PERSONNE_ID`':
                        $stmt->bindValue($identifier, $this->personne_id, PDO::PARAM_INT);
                        break;
                    case '`DEBUT`':
                        $stmt->bindValue($identifier, $this->debut, PDO::PARAM_STR);
                        break;
                    case '`FIN`':
    ...
    {
        if (isset($alreadyDumpedObjects['Cotisation'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Cotisation'][$this->getPrimaryKey()] = true;
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aPersonne) {
                $result['Personne'] = $this->aPersonne->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    {
        if ($this->aPersonne === null && ($this->personne_id !== null)) {
            $this->aPersonne = PersonneQuery::create()->findPk($this->personne_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPersonne->addCotisations($this);
             */
        }
    ...
    {
        return (string) $this->exportTo(CotisationPeer::DEFAULT_STRING_FORMAT);
    }
